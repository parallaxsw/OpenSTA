...
PropertyTypeNotComparable::PropertyTypeNotComparable(const char *type) :
  Exception(),
  msg_(sta::format("property type {} is not comparable.", type))
{
}
...
PropertiesNotComparable::PropertiesNotComparable(const char *type1,
                                                 const char *type2) :
  Exception(),
  msg_(sta::format("properties of type {} and {} are not comparable",
                  type1,
                  type2))
{
}
...
static std::string_view strcmpnat_tok_r(const char **head) {
  const char *start = *head;
  const char *end = start + strlen(start);
  const char *seeker = start;
  bool in_number = (start != end) && isdigit(*start);
  while (seeker != end && in_number == isdigit(*seeker))
    seeker++;
  *head = seeker;
  return std::string_view(start, seeker - start);
}

int strcmpnat(const char *lhs, const char *rhs) {
  const char *l_rem = lhs, *r_rem = rhs;
  ...
  while (l_tok.size() || r_tok.size()) {
    ...
    if (l_res.ec != std::errc{} && r_res.ec != std::errc{}) {
      int res = l_tok.compare(r_tok);
      if (res != 0)
        return res;
    }
    else if (l_res.ec != std::errc{})
      return -1;
    else if (r_res.ec != std::errc{})
      return 1;
    else {
      if (l_value != r_value)
        return l_value > r_value ? 1: -1;
      // e.g. comparing 01 and 0001
      int res = l_tok.compare(r_tok);
      if (res != 0)
          return res;
    }
    l_tok = strcmpnat_tok_r(&l_rem);
    r_tok = strcmpnat_tok_r(&r_rem);
  }
  return 0;
}

int PropertyValue::compare(const PropertyValue &rhs, const Network *network, bool natural) const {
  if (type_ != rhs.type_) {
    throw PropertiesNotComparable(type_name(type_), type_name(rhs.type_));
  }
  
  std::function<int(const char*, const char*)> my_strcmp = natural ? strcmpnat :
                                                                     strcmp;
  ...
  case Type::none:
    return 0;
  case Type::string:
    return my_strcmp(string_->c_str(), rhs.string_->c_str());
  case Type::float_:
  ...
  case Type::clk:
    ...
    return my_strcmp(lhs_s.c_str(), rhs_s.c_str());
  default:
...
PropertyValue::to_string(const Network *network) const
{
  ...
  case Type::bool_:
    ...
    if (bool_)
      return Sta::sta()->booleanPropsAsInt() ? "1" : "true";
    else
      return Sta::sta()->booleanPropsAsInt() ? "0" : "false";
  case Type::liberty_library:
                        ...
                        std::string_view property)
{
  ...
  else if (property == "valid_purposes")
                        ...
                        std::string_view property)
{
  ...
  if (property == "name"
    ...
    std::string_view name = network->direction(port)->name();
    if (sta_->directionPropsShort()) {
                        ...
                        std::string_view property)
{
  if (property == "name")
    ...
    return PropertyValue(port->libertyCell());
           ...
           || property == "port_direction") {
    std::string_view name = port->direction()->name();
    if (sta_->directionPropsShort()) {
      if (stringEqual(name, "input"))
        return PropertyValue("in");
      else if (stringEqual(name, "output"))
                        ...
                        std::string_view property)
{
  ...
  if (property == "name"
    ...
    std::string_view name = network->direction(pin)->name();
    if (sta_->directionPropsShort()) {
      if (stringEqual(name, "input"))
        return PropertyValue("in");
      else if (stringEqual(name, "output"))
