...
{
  findNext(to_level);
  return levelLessOrEqual(first_level_, last_level_) && !queue_[first_level_].empty();
}
...
{
  debugPrint(debug_, "bfs", 2, "enqueue %s", vertex->to_string(this).c_str());
    ...
    for (Vertex *v : queue_[level]) {
      if (v == vertex) {
        if (vertex->bfsInQueue(bfs_index_))
          ...
          debugPrint(debug_, "bfs", 1, "extra %s", vertex->to_string(this).c_str());
      }
    }
  }
  if (vertex->bfsInQueue(bfs_index_))
    debugPrint(debug_, "brs", 1, "missing %s", vertex->to_string(this).c_str());
}
...
BfsIterator::remove(Vertex *vertex)
{
  ...
  if (vertex->bfsInQueue(bfs_index_) && static_cast<Level>(queue_.size()) > level) {
    debugPrint(debug_, "bfs", 2, "remove %s", vertex->to_string(this).c_str());
    for (Vertex *&v : queue_[level]) {
      if (v == vertex) {
        v = nullptr;
        vertex->setBfsInQueue(bfs_index_, false);
