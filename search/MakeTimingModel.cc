...
MakeTimingModel::findTimingFromInputs()
{
  ...
  for (size_t begin = 0; begin < input_pins.size(); begin += batch_size) {
    ...
    PinSeq batch(input_pins.begin() + begin,
...
void
MakeTimingModel::findTimingFromInputsBatch(const PinSeq &batch,
                                           const ClockSeq &clks)
{
  ...
  BatchClockEdgeDelays batch_margins;
  ...
  BatchOutputPinDelays batch_output_delays;
...
MakeTimingModel::findOutputDelays(const ModelClockSources &clk_sources,
                                  BatchOutputPinDelays &output_delays)
{
      ...
      while (path_iter.hasNext()) {
        ...
        if (src_clk) {
          ...
          if (src_itr != clk_sources.end()) {
            auto [input_pin, input_rf] = src_itr->second;
