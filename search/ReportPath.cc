...
inline std::string getEndpointName(const StaState *state,
...
inline std::string getBusName(const StaState *state, 
  ...
  std::string endpoint_name = getEndpointName(state, sdc_network, end);
  bool is_bus;
  std::string bus_name;
  int index;
...
ReportPath::reportPathEnds(const PathEndSeq *ends) const
{
  ...
  if (ends && !ends->empty()) {
    ...
    PathEndSeq::const_iterator end_iter = ends->begin();
    PathEndSeq::const_iterator end_iter_end = ends->end(); // maybe ends wasn't the best way to name them
    
    std::set<PathEnd *> qualified_endpoints;
    ...
    if (silimate_dedup_endpoints_rx_.has_value()) {
      ...
      std::map<std::string, PathEnd *> worst_slack_by_bus;
      while (end_iter != end_iter_end) {
        PathEnd *end = *end_iter++;
