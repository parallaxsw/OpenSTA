...
using BatchOutputPinDelays = std::map<const Pin*, OutputPinDelays>;
using BatchClockEdgeDelays = std::map<const Pin*, ClockEdgeDelays>;
// Virtual clock -> (input pin, input transition) mapping used during
...
private:
  ...
  void findTimingFromInputsBatch(const PinSeq &batch,
                                 const ClockSeq &clks);
