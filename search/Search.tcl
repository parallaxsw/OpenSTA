...
See `set_false_path` for a description of allowed from_list, through_list and to_list objects.} \
  -arg_help {
    ...
    -dedup_by_word {Deduplicate reported paths that differ only by word.}
    -dedup_same_delay {Deduplicate reported paths that have the same delay.}
    -silimate_dedup_endpoints_rx {`regular_expression`: Deduplicate endpoints whose names match this regular expression.}
  }
...
define_cmd_alias "set_disable_clock_gating_check" \
...
define_cmd_alias "unset_disable_clock_gating_check" \
