...
paths are written in files named `<spice_file>_<path_number>.sp`.
...
The following commands now support a `-report_variance` argument.
...
The `define_scene` `-library` argument now takes the library name or a
...
The `define_corners` command is supported for compatibility but should
not be used with mcmm flows. Similarly, the `-min`/`-max` arguments to
`read_liberty` and `read_spef` are supported for compatibility but should
...
append commands to a mode by using the `-mode` argument for each
one. If no `-mode` argument is used the commands are added to the
current mode.
...
to annotate hierarchical blocks. Scene definitions use the spef name
to specify which parasitics to use for each scene.
...
The `read_spef` `-corner`/`-min`/`-max` arguments are supported for compatibility
but will be removed in a future release.

The `read_spef` `-reduce` options don't work because sdc, liberty ap isn't known.

The `report_checks` and `report_check_types` commands support a `-scenes`
...
VCD annotation with `read_vcd` now supports a `-mode` argument.
...
`-verbose` arguments and redirection. Use the `include` command instead.
...
Unlike the Tcl `source` command, `include` expands filenames with tilde `~`
and respects `sta_continue_on_error`.

## Release 2.6.0 2024/07/22

The version of C++ used by OpenSTA is now 17.
The Cudd BDD package is now required.
...
is now supported by the `read_saif` command.
...
delay calculator reduces the parasitic network to the appropriate type
and deletes the parasitic network.

The `report_clock_skew` and `report_clock_latency` commands now support
the `-include_internal_latency` flag to control inclusion of macro
internal clock delays from the cell liberty `min_clock_tree_path` /
`max_clock_tree_path` timing groups.
...
The `report_activity_annotation` command reports power activity annotation
...
The `read_liberty` command latch inference (see the command documentation) is now disabled by default.
...
Tcl readline support added. See the repository `README.md` for build instructions.
...
The `report_units` command reports the current units used by commands.
...
`-max_capacitance`, and `-min_capacitance`.
...
spice netlist is written in `path_<id>.sp` and the subckt file is
`path_<id>.subckt`.
...
compatibility in the current release.
