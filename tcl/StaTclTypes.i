                  ...
                  const Network *network)
{
  ...
  if (Tcl_ListObjGetElements(interp, source, &argc, &argv) == TCL_OK
    ...
    for (int i = 0; i < argc; i++) {
      void *obj;
      // check if sequence
      int res = SWIG_ConvertPtr(argv[i], &obj, seq_swig_type, false);
      if (SWIG_IsOK(res)) {
        auto collection = reinterpret_cast<SEQ_TYPE*>(obj);
        if (collection) {
...
%typemap(typecheck, precedence=SWIG_TYPECHECK_LIST) StringSeq {
  $1 = true;
}
