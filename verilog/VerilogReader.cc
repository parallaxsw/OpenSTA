...
{
  if (lib_cell && lib_cell->generatedClocks().size() > 0) {
    for (GeneratedClock *generated_clock : lib_cell->generatedClocks()) {
      ...
      std::string_view masterPin{generated_clock->masterPin()};
      std::string pinPath =  sta::format("{}/{}", inst_path_p, masterPin);

      // Map the full pinpath of source clock to the liberty cell
      // containing the generated clock definition
      network_->addGeneratedClockPinToCell(pinPath.c_str(), lib_cell);

      debugPrint(debug_, "libgenclk", 1, "Adding generated clock pin {} "
        "to liberty cell {} for instance {}",
