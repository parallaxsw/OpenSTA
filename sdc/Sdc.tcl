...
################################################################
# Executes the logical tree generated by the parser.
proc evaluate_ast {ast objects filter_function object_type} {
  set node_type [lindex $ast 0]
  
  if {$node_type eq "OR"} {
    set left_res  [evaluate_ast [lindex $ast 1] $objects $filter_function $object_type]
    set right_res [evaluate_ast [lindex $ast 2] $objects $filter_function $object_type]
    # Merge the lists and strip out any duplicates
    return [lsort -unique [concat $left_res $right_res]]
    
  } elseif {$node_type eq "AND"} {
    set left_res  [evaluate_ast [lindex $ast 1] $objects $filter_function $object_type]
    # Pass 'left_res' into the right evaluation to filter down further
    set right_res [evaluate_ast [lindex $ast 2] $left_res $filter_function $object_type]
    return $right_res
    
  } elseif {$node_type eq "COMP"} {
    set op   [lindex $ast 1]
    set attr [lindex $ast 2]
    set val  [lindex $ast 3]
    return [$filter_function $attr $op $val $objects]
    
  } elseif {$node_type eq "BOOL"} {
    set attr [lindex $ast 1]
    # Default boolean check per original logic
    return [$filter_function $attr "==" "1" $objects]
    
  } else {
    error "Unknown AST node type: $node_type"
  }
}

# Recurisve descent parser for filter_objs expressions
proc parse_or_expr {tokens} {
  # Parse the left side
  lassign [parse_and_expr $tokens] left_ast tokens
  
  # If we see an OR operator, parse the right side and join them in an OR node
  while {[llength $tokens] > 0 && [lindex $tokens 0] eq "||"} {
    set tokens [lrange $tokens 1 end] ; # Consume '||'
    lassign [parse_and_expr $tokens] right_ast tokens
    set left_ast [list OR $left_ast $right_ast]
  }
  return [list $left_ast $tokens]
}

proc parse_and_expr {tokens} {
  # Parse the primary unit (Parentheses or basic comparison)
  lassign [parse_primary $tokens] left_ast tokens
  
  # If we see an AND operator, parse the right side and join them in an AND node
  while {[llength $tokens] > 0 && [lindex $tokens 0] eq "&&"} {
    set tokens [lrange $tokens 1 end] ;# Consume '&&'
    lassign [parse_primary $tokens] right_ast tokens
    set left_ast [list AND $left_ast $right_ast]
  }
  return [list $left_ast $tokens]
}

proc parse_primary {tokens} {
  if {[llength $tokens] == 0} { error "Unexpected end of expression." }
  set tok [lindex $tokens 0]
  
  # Handle Parentheses
  if {$tok eq "("} {
    set tokens [lrange $tokens 1 end] ;# Consume '('
    lassign [parse_or_expr $tokens] expr_ast tokens
    
    if {[llength $tokens] == 0 || [lindex $tokens 0] ne ")"} {
      error "Missing closing parenthesis ')'."
    }
    set tokens [lrange $tokens 1 end] ;# Consume ')'
    return [list $expr_ast $tokens]
  }
  
  # Handle basic comparisons or boolean attributes
  set attr $tok
  set tokens [lrange $tokens 1 end] ;# Consume attribute name
  
  if {[llength $tokens] > 0} {
    set next_tok [lindex $tokens 0]
    if {$next_tok in {== != =~ !~}} {
      set op $next_tok
      set tokens [lrange $tokens 1 end] ;# Consume operator
      
      if {[llength $tokens] == 0} { error "Missing value after operator '$op'." }
      set val [lindex $tokens 0]
      set tokens [lrange $tokens 1 end] ;# Consume value
      
      return [list [list COMP $op $attr $val] $tokens]
    }
  }
  
  # If no operator follows, treat as a boolean type attribute
  return [list [list BOOL $attr] $tokens]
}

# Generic get_* filter.
proc filter_objs { filter objects filter_function object_type } {
  # Lexer
  # This regex pulls out logical ops, comp ops, parentheses, and identifiers/values.
  set raw_tokens [regexp -all -indices -inline {\s+|&&|\|\||==|!=|=~|!~|\(|\)|@?[a-zA-Z0-9_\*]+} $filter]
  
  # validate tokenization
  set previous_ending_index -1
  set validated_tokens {}
  foreach tok $raw_tokens {
    set start_index [lindex $tok 0]
    set end_index [lindex $tok 1]

    if {[expr {$previous_ending_index != [expr {$start_index - 1}]}]} {
      set invalid_range_begin [expr {$previous_ending_index + 1}]
      set invalid_range_end [expr {$start_index - 1}]
      set invalid_token [string range $filter $invalid_range_begin $invalid_range_end]
      sta_error 336 "Syntax error in $object_type -filter expression: Invalid token \"$invalid_token\" at column range $invalid_range_begin:$invalid_range_end"
    }

    set valid_token [string range $filter $start_index $end_index]
    set previous_ending_index $end_index
    lappend validated_tokens $valid_token
  }

  set tokens {}
  foreach tok $validated_tokens {
    # Drop pure whitespace tokens
    if {![string is space $tok]} {
      lappend tokens $tok
    }
  }

  if {[llength $tokens] == 0} { return $objects }

  # Parse into AST
  if {[catch {lassign [parse_or_expr $tokens] ast remaining_tokens} err]} {
    sta_error 337 "Syntax error in $object_type -filter expression: $err"
    return {}
  }
  
  if {[llength $remaining_tokens] > 0} {
    sta_error 338 "Syntax error in $object_type -filter expression: Extra tokens at end -> $remaining_tokens"
    return {}
  }

  # Evaluate AST
  return [evaluate_ast $ast $objects $filter_function $object_type]
}
